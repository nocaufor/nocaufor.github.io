...
- 首页 3D 主视觉：本地化的 `model-viewer` 组件 + 站点自有 glTF 轨道几何（太阳 / 地球 / 月球 + 两条细线轨道），
  缎面金属材质 + 本地 CC0 摄影棚 HDRI 环境柔光；太阳球体表面为纯净缎面金属（无经纬网格线、无凸起装饰），
  太阳以 45°/s 自转（8s 一周），地球沿外圈轨道 15°/s 公转（24s 一周），月球沿内圈轨道 40°/s 公转（9s 一周），
  三者由同一条 glTF 动画 `NOCAUMotion` 驱动，经组件 `autoplay` 常驻循环播放，任意主题下清晰可辨；
