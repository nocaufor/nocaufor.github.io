/* ============================================================
   nocau · 3D 项目星空引擎 v3（极简宇宙美学优化）
   深蓝近黑渐变 + 倾斜椭圆光带 + 中心星团/十字光效 + 全局鼠标控制 + 光爆进入过渡
...
(function () {
  ...
  /* 星点精选：性能约束粒子≤40；A/F/I 组全保留，其余按 size+heat 降序补齐 */
  function pickStars(all) {
    var rank = { A: 0, F: 1, I: 1, C: 2, B: 3 };
    var order = [];
    for (var i = 0; i < all.length; i++) {
      var g = (all[i].code || "B").charAt(0);
      order.push({ idx: i, g: rank[g] !== undefined ? rank[g] : 4, s: all[i].size + (all[i].heat || 0) * 0.4 });
    }
    order.sort(function (a, b) { return a.g - b.g || b.s - a.s; });
    var out = [];
    for (var j = 0; j < order.length && out.length < 40; j++) out.push(all[order[j].idx]);
    return out;
  }
  var stars = pickStars(_allStars);
  var N = stars.length;
  if (!N) return;
  ...
  /* ---------- 下滑衔接：首页首屏向下滚动 → 自动吸附星空并触发大爆炸 ----------
     滚动触发 + 节流防重入；每次下滑进入都稳定播放，无一次性状态残留 */
  var snapLock = false;
  ...
  function onPageScroll() {
    ...
    requestAnimationFrame(function () {
      ...
      var dy = y - prevScrollY;
      prevScrollY = y;
      var r = section.getBoundingClientRect();
      if (Math.abs(r.top) < 4) { snapLock = false; return; }
      if (snapLock) return;
      if (dy > 1 && r.top > 0 && r.top < window.innerHeight * 0.82) {
        snapLock = true;
        triggerLightBurst();
        window.scrollTo({ top: y + r.top, behavior: reduced ? "auto" : "smooth" });
      }
    });
  }
  ...
  function endPointer(e) {
    ...
    if (pointerState.active <= 1) pointerState.active = 1;
    ...
    if (pointerState.active === 1) pointerState.pinch = 0;
  }
  ...
  section.addEventListener("dblclick", function (e) {
    e.preventDefault();
    var hit = hitTest(pos(e).x, pos(e).y);
    if (hit) {
      /* 双击星星：打开详情 */
      openPanel(hit.st);
    } else {
  ...
  document.addEventListener("click", function (e) {
    ...
    if (t && t.closest && t.closest("[data-lang-toggle]")) {
      setTimeout(function () {
        if (currentStar) openPanel(currentStar);
        renderFrame(performance.now());
      }, 0);
    }
  });
